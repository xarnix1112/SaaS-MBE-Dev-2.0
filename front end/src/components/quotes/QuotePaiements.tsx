...
export function QuotePaiements({ devisId, quote: initialQuote, refreshKey }: QuotePaiementsProps) {
  ...
  // Calcul du total du devis de base (sans les surcoûts) - mémorisé avec useCallback
  // Ne dépend pas de paiements pour éviter les dépendances circulaires
  const calculateBaseQuoteTotal = useCallback(() => {
    if (!quote) return 0;
  ...
  const calculateQuoteTotal = useCallback(() => {
    const baseTotal = calculateBaseQuoteTotal();
  ...
  }, [calculateBaseQuoteTotal, paiements]);
  ...
  // Utilise calculateBaseQuoteTotal pour éviter la dépendance circulaire avec paiements
  const autoGeneratePrincipalPayment = useCallback(async () => {
    ...
    // Calculer le total actuel du devis (base + surcoûts actuels)
    const baseTotal = calculateBaseQuoteTotal();
    const surchargeAmount = paiements
      .filter((p) => p.type === 'SURCOUT' && p.status !== 'CANCELLED')
      .reduce((sum, p) => sum + p.amount, 0);
    const currentQuoteTotal = baseTotal + surchargeAmount;
  ...
  }, [quote, isAutoGenerating, paiements, devisId, loadPaiements, calculateBaseQuoteTotal]);
  ...
  // Utiliser un useEffect séparé pour éviter les dépendances circulaires
  useEffect(() => {
    if (!isLoading && quote && !hasAutoGeneratedRef.current) {
      // Vérifier s'il existe déjà un paiement principal EN ATTENTE
      const hasPendingPrincipalPayment = paiements.some(
        p => p.type === 'PRINCIPAL' && p.status === 'PENDING'
      );
      
      if (hasPendingPrincipalPayment) {
      ...
      const timer = setTimeout(async () => {
        if (hasAutoGeneratedRef.current || !quote) return;
        
        hasAutoGeneratedRef.current = true;
        
        // Calculer le total en utilisant les valeurs actuelles
        const baseTotal = calculateBaseQuoteTotal();
        const surchargeAmount = paiements
          .filter((p) => p.type === 'SURCOUT' && p.status !== 'CANCELLED')
          .reduce((sum, p) => sum + p.amount, 0);
        ...
        try {
          ...
          console.error('[QuotePaiements] ❌ Erreur génération automatique:', error);
          hasAutoGeneratedRef.current = false; // Réinitialiser en cas d'erreur
        } finally {
          setIsAutoGenerating(false);
        }
      }, 500);
  ...
  }, [isLoading, quote, paiements.length, refreshKey, calculateBaseQuoteTotal, devisId, loadPaiements]); // Ajouter toutes les dépendances nécessaires
  
  // Réinitialiser le flag quand refreshKey change pour permettre une nouvelle génération
  useEffect(() => {
    if (refreshKey !== undefined && refreshKey > 0) {
      hasAutoGeneratedRef.current = false;
    }
  ...
  // Réinitialiser le flag quand refreshKey change
  useEffect(() => {
    if (refreshKey !== undefined && refreshKey > 0) {
      hasAutoGeneratedRef.current = false;
    }
