...
| "client is offline" / unavailable | Règles Firestore bloquent tout (`if false`) ou **domaine non autorisé** | 1) Firebase Console → Authentication → Settings → **Authorized domains** : ajouter `staging.mbe-sdv.fr` ; 2) Activer Cloud Firestore API + Identity Toolkit API |
| 500 "5 NOT_FOUND" sur création compte | Backend Firebase ou clé API frontend | 1) Railway logs : voir quelle étape échoue (Étape 1-5) ; 2) Google Cloud → Identifiants → clé navigateur : ajouter **Identity Toolkit API** aux APIs autorisées (en plus de Cloud Firestore) ; 3) Vérifier rôles du service account (Firebase Admin) |
